...
from backend.db.sqlite import SessionLocal  # Fix #3: was missing, caused NameError
...
def detect_contradictions_task():
    """
    Compare concept pairs for contradictions (O(N^2), limited to 50 concepts for safety).
    Fix #12: llm_reasoning.check_contradiction() now returns a dict directly.
    """
    ...
    for i in range(len(concepts)):
        for j in range(i + 1, len(concepts)):
            ...
            # Fix #12: check_contradiction now returns dict (or None), not raw string
            check = llm_reasoning.check_contradiction(c1["properties"], c2["properties"])
            if check and check.get("contradiction"):
                try:
                    with driver.session() as session:
                        q_conflict = f"""
                        ...
                        session.run(
                            q_conflict,
                            id1=c1["id"], id2=c2["id"],
                            expl=check.get("explanation", "Detected by LLM")
                        )
                    conflicts += 1
                except Exception as e:
                    logger.error(f"Failed to write conflict edge: {e}")
...
def synthesize_ideas_task(concept_names: list, mode: str, project_id: int = None):
    """
    Fix #12: llm_reasoning.synthesize_idea_advanced() now returns a parsed dict directly.
    Fix #9:  Re-assign driver after connect() so it's never stale-None.
    Fix #14: Store neo4j_element_id in SQLite Idea for reliable future lookups.
    """
    # Fetch concept details
    concepts = [graph_service.get_concept_details(name) for name in concept_names]
    concepts = [c for c in concepts if c]

    # Call Advanced LLM — now returns dict or None
    ...
    if result:
        ...
        feasibility = result.get("feasibility_score", 0.5)

        driver = neo4j_client.driver
        if not driver:
            neo4j_client.connect()
            driver = neo4j_client.driver  # Fix #9

        with driver.session() as session:
            # 1. Create Idea Node in Neo4j
            q_idea = f"""
            CREATE (i:{Label.Idea} {{
                title: $title,
                summary: $summary,
            ...
            """
            idea_res = session.run(
                q_idea,
                title=title, summary=summary,
                novelty=novelty, feasibility=feasibility, mode=mode
            ).single()
            idea_id = idea_res["id"]  # Neo4j element ID (string)
        ...
        # Fix #14: also store neo4j_element_id for reliable future graph lookups
        ...
        new_idea = Idea(
            ...
            neo4j_element_id=idea_id  # Fix #14: stored so promote doesn't rely on title match
        )
